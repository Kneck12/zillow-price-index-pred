...
```{r}
zri_VA <- full_df %>%
  filter(State == "VA") %>% filter_index(~"2016-12") %>%
  aggregate_key(City / zip, avg_rent = mean(avg_rent))
...
zri_VA_full <- full_df %>%
  filter(State == "VA") #%>% filter(zip == "22201")
#rob_plot %>% filter(zip == "22201")%>% autoplot(zri_VA_22201)

forecast::accuracy((fit_rob %>%
  forecast()), zri_VA_full)
```
